...
class NTManager:
    def __init__(self, server=None):
        ...
        quixsam_table = NetworkTables.getTable("localizer")
        ...
        self.targets_table = quixsam_table.getSubTable("targets")
        ...
        self.targets_table.addEntryListener(self.targets_received)
    ...
    def odometry_received(self, table, key, value, is_new):
        try:
            id_ = int(value[0])
            if id_ % 5 != 0:  # Only keep every 5th measurement (10 Hz)
                return
    ...
    def vision_received(self, table, key, value, is_new):
        try:
            ...
            elif np.isfinite(id_) and all(np.isfinite(value)) and (len(value) - 1) % 5 == 0:
                data = []
                for i in range(int((len(value) - 1) / 5)):
                    data.append(Vision(id_, value[5*i + 1], value[5*i + 2], value[5*i + 3]))
    ...
    def targets_received(self, table, key, value, is_new):
        self.targets[int(key)] = value
